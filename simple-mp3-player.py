...
def track_length(track_path):
    ...
    if track_extension:
        try:
            ...
            track_length = mutagen_track.info.length
        except:
            track_length = 0
            tkinter.messagebox.showwarning(
                title="Warning!", message=f"Audio file incorrect : {track_path}")
        finally:
            track_length_formated = time.strftime('%M:%S', time.gmtime(track_length))
...
track_pos_slider = Scale(track_pos_frame, length=400, from_=1, to=0,
                         orient=HORIZONTAL, resolution=.1, showvalue=False, command=change_vol)
